...
(defn get-name
  "This function takes in a user id and performs an sql query which finds the username, and returns it."
  [id]
...
(defn get-or-create-id
  "This function will call get-id and if it returns nil it will create a new user and return that id."
  [username]
...
(defn insert-genome-reference
  ...
  (let [uuid (uuid-gen) name (:name data) path (:path data) cb (get-or-create-id (str (attrs "uid") "@" (uid-domain)))]
...
(defn modify-genome-reference
  ...
  (def data (cc-json/body->json body))
  (log/warn "JSON Object Passed=" data)
  (let [uuid (:uuid data) name (:name data) cb (get-or-create-id (str (attrs "uid") "@" (uid-domain))) path (:path data)]
    (update genome_reference
