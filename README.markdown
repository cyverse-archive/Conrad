...
As always the endpoint is specified at the end...point of the url request. The genome endpoints that get data, are done intuitively via a GET request. This is the protocal that curl requests default to so it does not have to specified. (the -v switch just turns verbose mode ON, feel free to exclude it.)
