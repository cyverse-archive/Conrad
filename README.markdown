...
As always the endpoint is specified at the end...point of the url request. The genome endpoints that GET data, are done via a GET request, this is the default curl request so it does not have to specified (the -v switch just turn verbose mode ON, feel free to exclude it).

if you need an authentication token then append the command with `?proxyToken=$(cas-ticket)` where cas-ticket is the authentication token generated by ticket.py.

if you want the JSON output to actually be readable (you do!) just append `| python -mjson.tool`

All together the entire command will look like this:
...
Note how there are both 'deleted' and non 'deleted' entries returned because we asked for `all-genome-references/`. There will probably be a LOT more data, try it yourself!

#### Listing Specific Genome References

There's also functionality for retrieving specific genome references.
